...
X = pd.read_excel("Services_enseignants2021-22r.xlsx",sheet_name=0,header=0,index_col=0)
...
def service_det_html(LEnseignants, nom_enseignant, filout):
        ...
        for j in range(len(cours)):
            ...
            if (math.isnan (groupes) or math.isnan (nseances)):
                ...
                hetd = duree * nseances * groupes * TD_CM
            #----- écriture d'un cours
            filout.write("<h2>" + niveau + " " + "S" + str(semestre) + " " + code + " " + titre + "</h2>\n")
            filout.write("<p>Heures (eqtd) : " + timeToStr(hetd) + " hetd</p>")
            service += hetd
            #filout.write(str(salle) + " "+ str(jour) + " " + str(hdebut) + " " + "durée = "+str(duree)+"h" +"\n")
            if (isinstance (cours[j].hdebut, datetime.time)):
                hdebut = cours[j].hdebut
                filout.write("<p>salle "+str(salle) + " - les "+ str(jour) + " à " + str(hdebut.hour) + 
                             "h" + str(hdebut.minute) + " ; " + "durée = " + timeToStr(duree) + "h")
